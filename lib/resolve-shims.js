...
var resolve = module.exports = function resolveShims (file, messages, shimConfig, cb) {
  // find the package.json that defines browserify-shim config for this file
  // make package.json config optional when transform options are provided
  mothership(file, function (pack) { return !! shimConfig || !! pack['browserify-shim'] }, function (err, res) {
    ...
    var browserifyShim = extend(pack['browserify-shim'], shimConfig);

    // we cached this before which means it was also grouped by file
    var cached = null; //shimsCache[packageDir];
